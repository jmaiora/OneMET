...
function TabBar({ active, onChange, accent }) {
  ...
  return (
    <div style={{ position: 'absolute', bottom: 6, left: 16, right: 16, zIndex: 40,
        ...
        return (
          <div key={t.id} onClick={() => onChange(t.id)}
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, cursor: 'pointer', flex: 1,
              padding: '7px 0', borderRadius: 999, background: on ? accent + '24' : 'transparent' }}>
            <Icon name={t.icon} color={on ? accent : T.ink3} size={22} stroke={on ? 2.2 : 1.9} />
            <span style={{ fontSize: 10, fontWeight: on ? 700 : 500, color: on ? accent : T.ink2 }}>{t.label}</span>
          </div>
        );
      })}
    </div>
